...
def message_received(message):
    ...
    try:
        ...
        url = "%s/webhooks/rest/webhook" % core_url
        params = None
        if token:
            params = {"token": token}
        r = session.post(
...
def main(argv):
    global core_url, token
    username = ""
    password = ""
    discovery_url = ""

    usage = "rasa_s4b_channel.py -d <skype for business discovery url> -u <username> -p <password> -c <rasa core base url> -t <rasa core authentication token>"
    try:
        opts, args = getopt.getopt(argv, "hd:u:p:c:t:")
    ...
    if not discovery_url:
        print("Expected Skype for Business Discovery URL.")
        sys.exit()
    elif not username:
        print("Expected Skype for Business username.")
        sys.exit()
    elif not password:
        print("Expected Skype for Business password.")
        sys.exit()
    elif not core_url:
        print("Expected Rasa core base url.")
        sys.exit()

    bot = SkypeForBusinessBot(discovery_url, username, password)
    bot.set_message_callback(message_received)
    bot.loop()
